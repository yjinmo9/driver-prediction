...
def main() -> int:
    ...
    # 한국어 주석: 통합 모델이 있으면 우선 사용, 없으면 A/B 로드로 폴백
    use_combined = False
    preproc_A = ensemble_A = preproc_B = ensemble_B = None
    try:
        preproc_all, ensemble_all = load_combined_model()
        use_combined = True
    except Exception:
        preproc_A, ensemble_A, preproc_B, ensemble_B = load_models()
    
    if use_combined:
        # 한국어 주석: 통합 피처 생성 (A/B 모두 합쳐서 동일 전처리 적용)
        key = "Test_id"
        A_df = test_idx[test_idx["Test"] == "A"].merge(A_test_feat, on=key, how="left", validate="1:1")
        B_df = test_idx[test_idx["Test"] == "B"].merge(B_test_feat, on=key, how="left", validate="1:1")
        df = pd.concat([A_df, B_df], axis=0, ignore_index=True)

        drop_cols = [key] + (["Test"] if "Test" in df.columns else [])
        feature_cols = [c for c in df.columns if c not in drop_cols]
        df = add_rowwise_features(df, feature_cols)
        X = df.drop(columns=drop_cols, errors="ignore")
        X_t = preproc_all.transform(X)
        proba = np.clip(ensemble_all.predict_proba(X_t)[:, 1], 1e-7, 1 - 1e-7)
        sub = df[[key]].copy()
        sub["Label"] = proba
    else:
        # 한국어 주석: A/B 분리 모델 경로 (폴백)
